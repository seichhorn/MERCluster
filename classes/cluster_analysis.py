...
class ClusterAnalysis:
	...
	def selectK(self, plot = True):
		...
		bootstrapClustering = [x for x in onlyfiles if 'bootstrap' in x]
...
#TODO: add a check to make sure that the genes you're requesting are in the data to begin with, would be helpful to print the missing ones
		...
		clusterData = rawData.merge(self.clusterLabels, left_index = True, right_index = True)
		...
		if cutUnstable:
			clusterData = clusterData.merge(self.stability, left_index = True, right_index = True)
