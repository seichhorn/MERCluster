import argparse
import sys
sys.path.insert(0,'/n/home13/seichhorn/Python_code/MERCluster/classes')
import experiment
...
def bootstrapClustering():
	...
	ex1.filter(verbose = args.verbose, countsPercentileCutoffs = list(args.countsPercentileCutoffs),genesPercentileCutoffs = list(args.genesPercentileCutoffs), mitoPercentileMax = args.mitoPercentileMax, geneMin = args.geneMin)
	ex1.bootstrapCells(frac = args.bootstrapFrac):
	ex1.selectVaraibleGenes(preselectedGenesFile = args.preselectedGenesFile, dispersionMin = args.dispersionMinMaxThreshold[0], dispersionMax = args.dispersionMinMaxThreshold[1], dispersionThreshold = args.dispersionMinMaxThreshold[2])
	ex1.processData(regressOut=args.regressOut)
	if args.usePCA:
		ex1.selectPCs()
	ex1.computeNeighbors(kValue=args.kValue, usePCA=args.usePCA)
	ex1.cluster(resolution=args.resolution, clusterMin=args.clusterMin, trackIterations=args.trackIterations):
